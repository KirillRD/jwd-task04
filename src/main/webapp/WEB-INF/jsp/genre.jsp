                        ...
                        <table class="w3-table w3-striped w3-border w3-hoverable w3-margin-top">
                            ...
                            <c:forEach var="genre" items="${requestScope.genres}">
                                ...
                                >
                                    ...
                                    <td>
                                        <form action="controller" method="post" onsubmit="return confirm('<fmt:message key="message.confirm-delete.genre"/>');">
                                            <input type="hidden" name="command" value="delete-genre">
                                            <input type="hidden" name="genre_id" value="${genre.id}">
                                            <button class="link" type="submit"
                                                ${genre.genreIsUsed ? 'disabled' : ''}
                                            ><span class="material-icons-outlined
                                            ${genre.genreIsUsed ? 'w3-text-gray' : 'w3-text-red'}"
                                                   title="<fmt:message key="genre.delete-genre"/>">clear</span></button>
                                        </form>
                                    </td>
                                </tr>
                            </c:forEach>
