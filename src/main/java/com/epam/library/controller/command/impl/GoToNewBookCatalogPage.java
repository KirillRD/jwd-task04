...
public class GoToNewBookCatalogPage implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        logger.info(logMessageBuilder("Going to catalog page of new books started", request));
        ...
        try {
            ...
            logger.info(logMessageBuilder("Going to catalog page of new books was completed", request));

            RequestProvider.forward(PagePath.NEW_POPULAR_BOOK_CATALOG_PAGE, request, response);
        } catch (ServiceException e) {
            logger.error(logMessageBuilder("Error in data while going to catalog page of new books", request), e);
